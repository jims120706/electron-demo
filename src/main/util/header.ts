...
export default class RequestHeader {
  userAgent: string
  initHeader: Record<string, any>
  header: Record<string, any>
  static instance: RequestHeader

  static getIns(): RequestHeader {
  ...
  getRequestHeader(): typeof this.header {
    return this.header
  }
  ...
  // 解析cookie
  parseCookies(rawCookies): Record<string, any> {
    // 使用正则表达式匹配所有的cookie项
    const cookieRegex =
      /(?:(?!\b(?:Domain|Secure|Path|Expires|SameSite|HttpOnly)\b)[^;]+(?=[^;]+)?);?/g
  ...
  // 序列化cookie
  serializeCookies(cookiesObj): string {
    let s = ''
